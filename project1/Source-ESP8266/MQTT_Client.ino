...
// Corrigido na versão 1.1: Porta estava fixada em 1883, agora reconhece a porta do comando CONNMQTT
// Corrigido na versão 1.2: Redefinição de KEEPALIVE e SOCKET TIMEOUT dentro do setup(). Dispensa mudanças no header file.
...
#define VERSION "MQTT CLIENT v1.2" // Definição de versão
  ...
  if(isBrokerConnected) {
    ...
    if(!isWiFiConnected) {
      ...
      if(lastfield < 3) { // Menos de 3 parâmetros (podem haver 3 ou 5)
        ...
        MQTT.setServer(ip, porta); // Define endereço e porta do broker mas não conecta ainda
...
void setup() {
  Serial.begin(19200);
  MQTT.setKeepAlive(60); // Trying to avoid MQTT disconnection
  MQTT.setSocketTimeout(60); // Same thing
  enter = false;
  FieldClear();
}
